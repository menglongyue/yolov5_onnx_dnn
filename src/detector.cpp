...
void Detector::postProcess(Mat &img, Detection &detection, Colors &cl)
{
    // Mat img1;
    // img.copyTo(img1);
    // PadInfo padInfo = letterbox(img1, this->inSize, Scalar(114, 114, 114), this->_auto, false, true, 32);
    ...
    PadInfo padInfo = detection.info;
    float scale = padInfo.scale;
    int left = padInfo.left;
    int top = padInfo.top;
        ...
        for (int r = 0; r < out.rows; r++)
    {
        ...
        int t_xmin = (cx - w / 2 - left) / scale;
        int t_ymin = (cy - h / 2) / scale;
        int t_w = w / scale;
        int t_h = h / scale;
        // boxes.push_back(Rect(cx - w / 2, cy - h / 2, w, h));
        boxes.push_back(Rect(t_xmin, t_ymin, t_w, t_h));
        indices.push_back(r);
        ...
        for (int r = 0; r < out.rows; r++)
    {
        ...
        int t_xmin = (cx - w / 2) / scale;
        int t_ymin = ((cy - h / 2) - top) / scale;
        int t_w = w / scale;
        int t_h = h / scale;
        // boxes.push_back(Rect(cx - w / 2, cy - h / 2, w, h));
        boxes.push_back(Rect(t_xmin, t_ymin, t_w, t_h));
